...
func (a Auth) AuthorizeLessor(ctx *fiber.Ctx) error {
	...
	} else if user.ID > 0 && user.UserType == domain.LESSOR {
		ctx.Locals("user", user)
		return ctx.Next()
	} else {
		return ctx.Status(401).JSON(&fiber.Map{
			"message": "authorization failed",
			"reason":  errors.New("please join lessor program to manage rooms"),
		})
	}
